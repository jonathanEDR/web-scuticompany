      ...
      <div className="flex-1 overflow-y-auto p-4">
        {generatedStructure ? (
          <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
              <div>
                ...
                <h2 className="text-xl font-bold text-gray-900 mt-1">{generatedStructure.title}</h2>
              </div>
              <div>
                <span className="text-xs font-medium text-gray-500 uppercase">Meta Descripción</span>
                <p className="text-sm text-gray-700 mt-1">{generatedStructure.meta_description}</p>
              </div>
            </div>

            {/* Introducción */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <span className="text-xs font-medium text-blue-700 uppercase">Introducción</span>
              <p className="text-sm text-gray-700 mt-1">{generatedStructure.introduction}</p>
            </div>

            {/* Secciones */}
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-gray-900">Estructura del contenido</h4>
                ...
                <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 space-y-2">
                  <div className="flex items-start justify-between">
                    <h3 className={`font-semibold text-gray-900 ${
                      ...
                      <span className="text-blue-600 mr-2">{`H${section.level}`}</span>
                      {section.heading}
                    </h3>
                  ...
                  {section.keywords.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {section.keywords.map((keyword, kidx) => (
                        <span key={kidx} className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded">
                          {keyword}
                        </span>
                      ))}
                    </div>
            ...
            <div className="space-y-3">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <span className="text-xs font-medium text-green-700 uppercase">Conclusión</span>
                <p className="text-sm text-gray-700 mt-1">{generatedStructure.conclusion}</p>
              </div>
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <span className="text-xs font-medium text-purple-700 uppercase">Call to Action</span>
                <p className="text-sm text-gray-700 mt-1">{generatedStructure.cta}</p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-600">{generatedStructure.total_words}</div>
                  <div className="text-xs text-gray-600">Palabras totales</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-600">{generatedStructure.estimated_reading_time} min</div>
                  <div className="text-xs text-gray-600">Tiempo de lectura</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <h3 className="text-lg font-medium mb-2">Generador de Estructura</h3>
